...
import { getCloudFileStream } from "./cloudFile"
...
export async function generateOrganizationZipBuffer(
  organizationId: number
): Promise<{ buffer: Buffer; organizationName: string | null }> {
  const accountIds = (
  ...
  for (const policy of insurancePolicies) {
    const companyFolder =
      sanitizeName(policy.company) || "unknown-company"
  ...
  for (const orgCloudFile of orgCloudFiles) {
    ...
    const filePath = uniquePath(usedPaths, `files/${orgCloudFile.name}`)
    try {
  ...
  return { buffer: zipBuffer, organizationName: organization?.name ?? null }
}